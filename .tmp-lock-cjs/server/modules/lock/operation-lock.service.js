...
exports.DEFAULT_OPERATION_LOCK_TTL_MS = void 0;
exports.acquireLock = acquireLock;
exports.releaseLock = releaseLock;
exports.heartbeatLock = heartbeatLock;
...
const prisma_server_1 = __importDefault(require("../../db/prisma.server"));
const logger_1 = require("../../utils/logger");
const logger = (0, logger_1.createLogger)({ module: "shop-operation-lock" });
/** 默认 30 分钟超时。 */
exports.DEFAULT_OPERATION_LOCK_TTL_MS = 30 * 60 * 1000;
/**
 * 获取 shop 锁。
 *
 * 规则：
 ...
 * - 其他 RUNNING 锁：返回冲突
 */
async function acquireLock(shopId, operationType, owner, options) {
    const ttlMs = normalizeTtlMs(options?.ttlMs);
    return prisma_server_1.default.$transaction(async (tx) => {
        const now = new Date();
 ...
 */
async function releaseLock(shopId, owner) {
    return prisma_server_1.default.$transaction(async (tx) => {
 ...
 */
async function heartbeatLock(shopId, owner, options) {
    const ttlMs = normalizeTtlMs(options?.ttlMs);
    return prisma_server_1.default.$transaction(async (tx) => {
        const currentLock = await selectLockForUpdate(tx, shopId);
        if (currentLock === null) {
...
async function cleanupExpiredLocks() {
    const now = new Date();
    const cleanedRows = await prisma_server_1.default.$queryRaw(client_1.Prisma.sql `
...
function normalizeTtlMs(ttlMs) {
    const normalized = ttlMs ?? exports.DEFAULT_OPERATION_LOCK_TTL_MS;

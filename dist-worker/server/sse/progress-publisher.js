...
import { SCAN_PROGRESS_KEY_PREFIX, SCAN_PROGRESS_TTL_SECONDS, SCAN_PHASE, ALL_SCAN_RESOURCE_TYPES, } from "../modules/scan/scan.constants";
 ...
 * @param scanJobId scan_job 的主键
 */
export async function initScanProgress(scanJobId) {
    ...
    logger.info({ scanJobId, key }, "Redis scan progress initialized");
}
/**
 ...
 */
export async function addScanTotalImages(scanJobId, totalImages) {
    if (totalImages <= 0)
        return;
    const key = getScanProgressKey(scanJobId);
    const redis = queueConnection;
 ...
 */
export async function setScanDiscoveredObjects(scanJobId, discoveredObjects) {
    if (discoveredObjects < 0)
        return;
    const key = getScanProgressKey(scanJobId);
    const redis = queueConnection;
    ...
    await redis.expire(key, SCAN_PROGRESS_TTL_SECONDS);
    logger.info({ scanJobId, discoveredObjects }, "Redis scan discoveredObjects updated");
}
/**
 ...
 */
export async function incrementScanProcessedImages(scanJobId, processed, failed = 0) {
    const key = getScanProgressKey(scanJobId);
    const redis = queueConnection;
    if (processed > 0) {
    ...
    await redis.expire(key, SCAN_PROGRESS_TTL_SECONDS);
    logger.info({ scanJobId, processed, failed }, "Redis scan processedImages incremented");
}
/* ------------------------------------------------------------------ */
...
function resourceTotalsField(resourceType, metric) {
 ...
 */
async function readResourceTotals(scanJobId) {
    const key = getScanProgressKey(scanJobId);
    const redis = queueConnection;
 ...
 * @param totalImages 本次解析出的图片总数（增量）
 */
export async function addScanResourceTotalImages(scanJobId, resourceType, totalImages) {
    if (totalImages <= 0)
        return;
    const key = getScanProgressKey(scanJobId);
    const redis = queueConnection;
 ...
 * @param failed 失败图片数增量（可选）
 */
export async function addScanResourceProcessedImages(scanJobId, resourceType, processed, failed = 0) {
    const key = getScanProgressKey(scanJobId);
    const redis = queueConnection;
    const exists = await redis.exists(key);
    if (!exists)
...
export async function getScanProgress(scanJobId) {
    ...
    const etaSeconds = await estimateEtaSeconds(scanJobId, totalImages, processedImages + failedImages);
    return {
        totalImages,
        processedImages,
        failedImages,
 ...
 */
function getRateSampleKey(scanJobId) {
 ...
 */
async function estimateEtaSeconds(scanJobId, totalImages, doneImages) {
    if (totalImages <= 0)
        return null;
    ...
    const now = Date.now();
    try {
        const sample = { t: now, processed: doneImages };
 ...
 * @param batchId 批次 ID
 */
export function getGenerationProgressChannel(batchId) {
 ...
 */
export async function readGenerationProgress(batchId) {
    const key = getGenerationProgressKey(batchId);

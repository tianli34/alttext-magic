...
export class OpenAICompatibleProvider {
    ...
    async generateAlt(req) {
        ...
        try {
            ...
            throw new AIGenerationError(`[${providerName}] 网络请求失败: ${err.message}`, err);
        }
        finally {
            clearTimeout(timer);
        }
        ...
        if (response.status >= 500) {
            throw new AIGenerationError(`[${providerName}] 服务端错误: HTTP ${response.status}`);
        }
        if (!response.ok) {
            const text = await response.text().catch(() => "");
            throw new AIGenerationError(`[${providerName}] 请求失败: HTTP ${response.status} — ${text}`);
        }
        let data;
        try {
            ...
            throw new AIGenerationError(`[${providerName}] 响应格式异常（无法解析 JSON）`, err);
        }
        const content = data.choices?.[0]?.message?.content;
        if (!content || typeof content !== "string" || content.trim() === "") {
            throw new AIGenerationError(`[${providerName}] 响应格式异常（content 为空）`);
        }
        return {

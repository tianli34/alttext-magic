...
// Prisma 实现
// ---------------------------------------------------------------------------
function createPrismaDataAccess() {
    return {
 ...
 */
export async function updateDraftEditedText(shopId, candidateId, editedText, dataAccess = createPrismaDataAccess()) {
    // 1. 查找候选及其草稿
    const candidate = await dataAccess.getCandidateWithDraft(shopId, candidateId);
    if (!candidate) {
        throw new DraftUpdateError("CANDIDATE_NOT_FOUND", "候选不存在或不属于当前店铺", 404);
    }
    // 2. 确认关联草稿存在
    if (!candidate.draft) {
        throw new DraftUpdateError("NO_DRAFT", "该候选尚无关联草稿", 404);
    }
    // 3. 校验 candidate 状态是否允许编辑
    if (!EDITABLE_STATUSES.has(candidate.status)) {
        throw new DraftUpdateError("INVALID_STATUS", `当前状态 ${candidate.status} 不允许编辑，仅支持 GENERATED 和 WRITEBACK_FAILED_RETRYABLE`, 409);
    }
    // 4. 执行更新
    const updated = await dataAccess.updateDraftEditedText(candidate.draft.id, editedText);
    logger.info({ candidateId, draftId: updated.id }, "草稿编辑文本已更新");
    return updated;
}

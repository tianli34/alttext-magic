 ...
 */
import { DEFAULT_SCOPE_FLAG_STATE, EMPTY_SCOPE_FLAG_STATE, normalizeScopeFlagState, SCOPE_FLAG_ORDER, scopeFlagStateSchema, } from "../../../app/lib/scope-utils";
 ...
 */
export function computeEffectiveReadScopeFlags(scanScopeFlags, grantedReadScopes) {
    if (grantedReadScopes === null) {
        return { ...EMPTY_SCOPE_FLAG_STATE };
    }
    return SCOPE_FLAG_ORDER.reduce((acc, flag) => {
 ...
 */
export function normalizeScanScopeFlags(input) {
 ...
 */
export async function getScopeSettings(shopId) {
    ...
    if (!shop) {
        ...
        return {
            ...
            effectiveReadScopeFlags: computeEffectiveReadScopeFlags(DEFAULT_SCAN_SCOPE_FLAGS, null),
        };
    }
    ...
    const lastPublishedScopeFlags = shop.lastPublishedScopeFlags
        ...
        : null;
    const effectiveReadScopeFlags = computeEffectiveReadScopeFlags(scanScopeFlags, lastPublishedScopeFlags);
    return {
        scanScopeFlags,
 ...
 */
export async function updateScanScopeFlags(shopId, flags) {
    ...
    logger.info({ shopId, scanScopeFlags: normalized }, "Updated scan_scope_flags");
    ...
    const lastPublishedScopeFlags = shop.lastPublishedScopeFlags
        ? normalizeScopeFlagState(shop.lastPublishedScopeFlags)
        : null;
    const effectiveReadScopeFlags = computeEffectiveReadScopeFlags(normalized, lastPublishedScopeFlags);
    return {
        scanScopeFlags: normalized,
        lastPublishedScopeFlags,
        effectiveReadScopeFlags,

 ...
 */
export async function createScanJobWithTasks(params) {
    ...
    return prisma.$transaction(async (tx) => {
        ...
        const tasks = await Promise.all(enabledResourceTypes.map((resourceType) => tx.scanTask
            .create({
            data: {
                shopId,
 ...
 */
function buildLatestAttempt(attempts) {
...
export async function getScanStatus(scanJobId, shopId) {
    ...
    const scanJobDto = {
        ...
        successfulResourceTypes: scanJob.successfulResourceTypes,
        failedResourceTypes: scanJob.failedResourceTypes,
    ...
    const taskDtos = tasksWithAttempts.map((task) => ({
        id: task.id,

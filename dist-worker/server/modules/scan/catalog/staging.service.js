 ...
 * 批量写入 stg_article 表。
 */
export async function flushArticleStaging(shopId, scanTaskAttemptId, rows) {
    if (rows.length === 0)
        return 0;
    ...
    logger.debug({ shopId, scanTaskAttemptId, count: result.count, inputCount: rows.length }, "staging.article.flushed");
    return result.count;
}
/* ------------------------------------------------------------------ */
 ...
 */
export async function flushCollectionStaging(shopId, scanTaskAttemptId, rows) {
    if (rows.length === 0)
        return 0;
    ...
    logger.debug({ shopId, scanTaskAttemptId, count: result.count, inputCount: rows.length }, "staging.collection.flushed");
    return result.count;
}
/* ------------------------------------------------------------------ */
/*  Files staging flush                                                 */
 ...
 */
export async function flushMediaFileStaging(shopId, scanTaskAttemptId, rows) {
    if (rows.length === 0)
        return 0;
    ...
    logger.debug({ shopId, scanTaskAttemptId, count: result.count, inputCount: rows.length }, "staging.media-file.flushed");
    return result.count;
}
/* ------------------------------------------------------------------ */
 ...
 */
export async function flushProductMediaStaging(shopId, scanTaskAttemptId, items) {
    if (items.length === 0)
        return { productCount: 0, mediaCount: 0 };
    // 按 kind 分组
    const products = [];
    const mediaImages = [];
 ...
 */
export async function countStagingRows(scanTaskAttemptId, resourceType) {
    switch (resourceType) {
        case "ARTICLE_IMAGE":

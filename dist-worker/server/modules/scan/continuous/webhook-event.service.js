 ...
 */
export function isDebounceTopic(topic) {
 ...
 */
export async function markCoalesced(id, coalescedIntoEventId) {
...
export async function markFailed(id, error) {
    ...
    await prisma.webhookEvent
        ...
        .catch((updateErr) => {
        logger.error({ id, err: updateErr }, "webhook-event.mark_failed_update_err");
    });
...
export async function routeDebounceWebhook(params) {
    ...
    if (!resourceId) {
        logger.warn({ shopDomain, topic, webhookEventId }, "webhook-event-debounce.no_resource_id");
        return;
    }
    // 2. 查询 shopId
    ...
    if (!shop) {
        logger.warn({ shopDomain, topic, webhookEventId }, "webhook-event-debounce.shop_not_found");
        return;
    }
    ...
    await prisma.webhookEvent
        ...
        .catch((err) => {
        logger.warn({ webhookEventId, resourceId, err }, "webhook-event-debounce.resource_id_update_failed");
    });
    // 4. Debounce tryAcquire
    const result = await debounce.tryAcquire(shopId, topic, resourceId, webhookEventId, DEBOUNCE_WINDOW_SEC);
    if (result.acquired) {
        ...
        logger.info({ shopDomain, topic, resourceId, webhookEventId }, "webhook-event-debounce.acquired");
    }
    else {
        // 后续事件：更新 debounce key + 标记旧事件 COALESCED
        await debounce.update(shopId, topic, resourceId, webhookEventId, DEBOUNCE_WINDOW_SEC);

 ...
 */
export async function releaseLockByType(shopId, operationType) {
    return prisma.$transaction(async (tx) => {
        const currentLock = await selectLockForUpdate(tx, shopId);
        if (currentLock === null) {
            return {
        ...
        logger.info({ shopId, batchId: currentLock.batchId, operationType }, "Operation lock released by operation type");
        return {
            released: true,
            reason: "RELEASED",
            lock: releasedLock,
 ...
 */
export async function isOperationRunning(shopId, operationType) {
    const now = new Date();
 ...
 */
export async function hasRunningScanJob(shopId) {
    const rows = await prisma.$queryRaw(Prisma.sql `
    SELECT "id"
    FROM "scan_job"
    WHERE "shop_id" = ${shopId}

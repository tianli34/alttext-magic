 ...
 */
import { BILLING_INTERVALS, PLAN_KEYS } from './billing.types';
 ...
 */
export function getIncludedCredits(planKey, interval) {
 ...
 * FREE 返回 0。
 */
export function getAnnualIncludedCredits(planKey) {
 ...
 * @param interval 计费周期
 */
export function getMonthlyCycleKey(date, planKey, interval) {
    if (interval === 'ANNUAL') {
 ...
 */
export function getFreeCycleKey(date) {
    const year = date.getUTCFullYear();
    const month = String(date.getUTCMonth() + 1).padStart(2, '0');
 ...
 */
export function getPaidWelcomeCycleKey(planKey) {
    if (planKey === 'FREE') {
 ...
 */
export function getInstallWelcomeCycleKey() {
...
export function canPurchaseOveragePack(planKey) {
    const config = getPlanConfig(planKey);
...
// ============================================================================
/** 获取安装欢迎额度数量 */
export function getInstallWelcomeCredits() {
 ...
 * FREE 返回 0。
 */
export function getPaidWelcomeCredits(planKey) {
    if (planKey === 'FREE') {
        return 0;
    }

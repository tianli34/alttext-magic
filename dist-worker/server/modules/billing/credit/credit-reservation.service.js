/**
 * File: server/modules/billing/credit/credit-reservation.service.ts
...
import { sortBucketsByConsumptionOrder } from './consumption-order.js';
...
function buildReservationIdempotencyKey(shopId, batchId) {
    return `${shopId}:batch:${batchId}:reservation`;
}
function buildLedgerIdempotencyKey(reservationId, lineId, type) {
    return `${reservationId}:${lineId}:${type}`;
}
function toReservationWithLines(reservation) {
    return reservation;
}
async function loadReservationWithLines(tx, shopId, reservationId) {
...
async function lockReservation(tx, shopId, reservationId) {
    await tx.$queryRaw `
    SELECT id
...
async function lockSpendableBuckets(tx, shopId, now) {
...
async function getLockedSpendableBuckets(tx, shopId, now) {
    ...
    return sortBucketsByConsumptionOrder(buckets.map((bucket) => ({
        id: bucket.id,
...
function planReservationLines(buckets, amount) {
...
function resolveReleaseStatus(reason) {
    return reason.trim().toUpperCase() === 'EXPIRED' ? 'EXPIRED' : 'RELEASED';
}
export async function createReservation(params, client = prisma) {
...
export async function consumeReservation(params, client = prisma) {
...
export async function releaseReservation(params, client = prisma) {

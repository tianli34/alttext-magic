...
export async function changePlanToPaid(params, adapter) {
    ...
    log.info({ shopId, planKey, confirmationUrl: result.confirmationUrl }, '订阅确认链接创建成功');
    return {
    ...
    if (subsResult.success && subsResult.subscriptions.length > 0) {
        for (const sub of subsResult.subscriptions) {
            if (sub.status === 'ACTIVE') {
                log.info({ shopId, subscriptionId: sub.id }, '取消活跃 Shopify 订阅');
                ...
                if (cancelResult.success) {
                    ...
                    log.warn({ shopId, subscriptionId: sub.id, error: cancelResult.errorMessage }, '取消 Shopify 订阅失败，继续本地降级');
                }
            }
        }
    }

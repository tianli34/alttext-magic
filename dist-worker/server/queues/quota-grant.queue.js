...
function getQueue() {
    if (!_queue) {
        _queue = new Queue(QUOTA_GRANT_QUEUE_NAME, {
 ...
 */
export function getQuotaGrantQueue() {
    return getQueue();
}
/**
...
export async function enqueueQuotaGrant(data) {
    ...
    logger.info({ source: data.source, targetMonth: data.targetMonth }, "quota-grant.enqueue");
}

...
function getQueue() {
    if (!_queue) {
        _queue = new Queue(BILLING_SYNC_QUEUE_NAME, {
 ...
 */
export function getBillingSyncQueue() {
    return getQueue();
}
/**
 ...
 */
export async function enqueueBillingSync(data) {
    ...
    logger.info({ shopDomain: data.shopDomain, source: data.source }, "billing-sync.queue.enqueued");
}

...
import { isDebounceTopic, routeDebounceWebhook, } from "../../../../server/modules/scan/continuous/webhook-event.service";
...
export async function receiveWebhook(envelope) {
    ...
    if (receipt.isNew) {
        if (isDebounceTopic(envelope.topic)) {
            ...
            logger.info({ shop: envelope.shop, topic: envelope.topic, eventId: receipt.eventId }, "webhook.receive.debounce_routed");
        }
        else {
            // 其他 topic → 通用 webhook 队列
            await enqueueWebhookEvent({ webhookEventId: receipt.eventId });
            logger.info({ shop: envelope.shop, topic: envelope.topic, eventId: receipt.eventId }, "webhook.receive.enqueued");
        }
    }
    else {
        logger.info({ shop: envelope.shop, topic: envelope.topic, eventId: receipt.eventId }, "webhook.receive.duplicate_skipped");
    }
    return receipt;
}

 ...
 */
import { cleanupExpiredLocks, } from "../../server/modules/lock/operation-lock.service";
...
export async function runLockTimeoutCleanupOnce() {
    const result = await cleanupExpiredLocks();
    if (result.cleanedCount > 0) {
        logger.warn({ cleanedCount: result.cleanedCount }, "Lock timeout cleanup reclaimed expired locks");
    }
 ...
 * - Worker 启动时调用此函数即可。
 */
export async function registerLockReaperScheduler() {
    const queue = getLockReaperQueue();
    // 清理历史 5 分钟 repeatable 键，确保间隔切换幂等
    await queue.removeRepeatable("lock-reaper-scheduled", { every: LOCK_REAPER_OLD_EVERY_MS }, LOCK_REAPER_REPEATABLE_JOB_ID);
    await queue.add("lock-reaper-scheduled", { source: "scheduled" }, {
        repeat: {
    ...
    logger.info({ everyMs: LOCK_REAPER_EVERY_MS, jobId: LOCK_REAPER_REPEATABLE_JOB_ID }, "lock-reaper.scheduler.registered");
}

 ...
 * - Worker 启动时调用此函数即可。
 */
export async function registerReservationReaperScheduler() {
    const queue = getReservationReaperQueue();
    await queue.add("reservation-reaper-scheduled", { source: "scheduled" }, {
        repeat: {
            every: EVERY_MS,
        },
    ...
    logger.info({ everyMs: EVERY_MS, jobId: REPEATABLE_JOB_ID }, "reservation-reaper.scheduler.registered");
}

 ...
 * - Worker 启动时调用此函数即可。
 */
export async function registerFreeMonthlyGrantScheduler() {
    const queue = getQuotaGrantQueue();
    // 清理历史 daily jobId 的 repeatable 键，确保重命名迁移幂等
    await queue.removeRepeatable("quota-grant-scheduled", { pattern: DAILY_CRON }, OLD_REPEATABLE_JOB_ID);
    await queue.add("quota-grant-scheduled", { source: "scheduled" }, {
        repeat: {
            pattern: DAILY_CRON,
        },
    ...
    logger.info({ cron: DAILY_CRON, jobId: REPEATABLE_JOB_ID }, "free-monthly-grant.scheduler.registered");
}

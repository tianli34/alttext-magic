...
import { markProcessing, markProcessed, markSkipped, markFailed, } from "../../server/modules/scan/continuous/webhook-event.service";
 ...
 */
export async function processContinuousScanCollectionJob(job) {
    const { shopId, collectionId, latestWebhookEventId } = job.data;
    logger.info({ shopId, collectionId, latestWebhookEventId, jobId: job.id }, "continuous-scan-collection.processor.start");
    ...
    try {
        ...
        if (!planEnabled) {
            logger.info({ shopId, latestWebhookEventId }, "continuous-scan-collection.processor.skipped_plan");
        ...
        if (!scopeEnabled) {
            logger.info({ shopId, latestWebhookEventId }, "continuous-scan-collection.processor.skipped_scope");
        ...
        // 7. 计算图片指纹 (8-A3)
        const fp = computeCollectionFingerprint(collectionImage ? { url: collectionImage.url, altText: collectionImage.altText } : null);
        // 8. Gate 4：fingerprintGate 校验图片指纹是否变化 (8-C5)
        const fingerprintResult = await checkFingerprintChange(shopId, "COLLECTION", collectionId, fp);
        if (fingerprintResult === "UNCHANGED") {
            logger.info({ shopId, collectionId, latestWebhookEventId }, "continuous-scan-collection.processor.skipped_no_image_change");
        ...
        logger.info({ shopId, collectionId, latestWebhookEventId }, "continuous-scan-collection.processor.success");
    }
    catch (error) {
        logger.error({ shopId, collectionId, latestWebhookEventId, err: error }, "continuous-scan-collection.processor.failed");

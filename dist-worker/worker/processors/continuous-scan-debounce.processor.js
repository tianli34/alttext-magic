...
import { enqueueProductScan, enqueueCollectionScan, } from "../../server/queues/continuous-scan.queue";
 ...
 */
export async function processContinuousScanDebounceJob(data) {
    ...
    try {
        ...
        if (!latestWebhookEventId) {
            logger.warn({ shopId, topic, resourceId }, "debounce.processor.key_not_found");
            return;
        }
        switch (topic) {
            ...
            case "collections/update":
                ...
                break;
            default:
                logger.error({ shopId, topic, resourceId }, "debounce.processor.unknown_topic");
                return;
        }
        logger.info({ shopId, topic, resourceId, latestWebhookEventId }, "debounce.processor.completed");
    }
    catch (error) {
        logger.error({ shopId, topic, resourceId, err: error }, "debounce.processor.failed");
        throw error;
    }
}

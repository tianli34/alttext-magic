 ...
 */
export async function processReservationReaperJob(data, batchSize = DEFAULT_BATCH_SIZE) {
    const { source } = data;
    ...
    logger.info({ source, count: expiredReservations.length }, "reservation-reaper.processor.found-expired");
    let releasedCount = 0;
    let skippedCount = 0;
    let failedCount = 0;
    ...
    if (failedCount > 0 && releasedCount === 0 && expiredReservations.length > 0) {
        throw new Error(`[reservation-reaper] 所有 reservation 释放失败 (failures: ${failedCount})`);
    }
}

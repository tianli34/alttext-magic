 ...
 */
export async function processQuotaGrantJob(data) {
    ...
    try {
        ...
        if (result.failedCount > 0 && result.grantedCount === 0 && result.totalFreeShops > 0) {
            throw new Error(`[quota-grant] 所有店铺发放失败 (failures: ${result.failedCount})`);
        }
    }
    catch (error) {
        logger.error({ source, targetMonth, err: error }, "quota-grant.processor.failed");
        throw error;
    }
}

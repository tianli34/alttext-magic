...
export async function processWritebackJob(data, dependencies = defaultDependencies) {
    ...
    if (!claimed) {
        jobLogger.info({ shopId: data.shopId }, "writeback.job-item-terminal-skip");
        return;
    }
...
export async function finalizeBatchIfComplete(data, log = logger, dependencies = defaultDependencies) {
    ...
    if (!batch || batch.status !== JobBatchStatus.RUNNING)
        return;
...
async function loadCandidate(data, client) {
...
function resolveFinalBatchStatus(batch) {
    if (batch.failed === 0)
        return JobBatchStatus.SUCCESS;
    if (batch.success + batch.skipped > 0)
        return JobBatchStatus.PARTIAL_SUCCESS;

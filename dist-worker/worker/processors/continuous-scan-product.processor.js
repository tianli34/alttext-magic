 ...
 */
export async function processContinuousScanProductJob(job) {
    const { shopId, productId, latestWebhookEventId } = job.data;
    logger.info({ shopId, productId, latestWebhookEventId, jobId: job.id }, "continuous-scan-product.processor.start");
    ...
    try {
        ...
        if (!planEnabled) {
            logger.info({ shopId, latestWebhookEventId }, "continuous-scan-product.processor.skipped_plan");
        ...
        if (!scopeEnabled) {
            logger.info({ shopId, latestWebhookEventId }, "continuous-scan-product.processor.skipped_scope");
        ...
        const fingerprintResult = await checkFingerprintChange(shopId, "PRODUCT", productId, fp);
        if (fingerprintResult === "UNCHANGED") {
            logger.info({ shopId, productId, latestWebhookEventId }, "continuous-scan-product.processor.skipped_no_image_change");
        ...
        logger.info({ shopId, productId, latestWebhookEventId }, "continuous-scan-product.processor.success");
    }
    catch (error) {
        logger.error({ shopId, productId, latestWebhookEventId, err: error }, "continuous-scan-product.processor.failed");

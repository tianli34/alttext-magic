...
const parseBulkProcessorDependencies = {
    ...defaultDependencies,
};
export function setParseBulkProcessorDependenciesForTests(overrides) {
    Object.assign(parseBulkProcessorDependencies, overrides);
}
export function resetParseBulkProcessorDependenciesForTests() {
...
export async function processParseBulkJob(data) {
    ...
    if (!attempt.bulkResultUrl) {
        throw new Error(`ScanTaskAttempt ${scanTaskAttemptId} has no bulkResultUrl`);
    }
    ...
    await updateScanProgressPhase(scanJobId, SCAN_PHASE.PARSING, `正在解析 ${resourceType} 数据…`);
    try {
        ...
        if (failure.retryable &&
            ...
            const terminalErrorMessage = retrySubmitResult.status === "failed"
            ...
            const finalizeResult = await parseBulkProcessorDependencies.finalizeScanJobIfTerminal(scanJobId);
...
/*  按资源类型分发解析                                                   */
/* ------------------------------------------------------------------ */
async function parseByResourceType(shopId, scanTaskAttemptId, resourceType, bulkResultUrl) {
    switch (resourceType) {
        case "ARTICLE_IMAGE": {

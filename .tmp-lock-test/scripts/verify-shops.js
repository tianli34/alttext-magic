...
function decryptToken(encryptedToken, nonce, tag) {
...
async function main() {
    ...
    const shops = await prisma.$queryRaw(Prisma.sql `
      SELECT
    ...
    for (const shop of shops) {
        ...
        if (JSON.stringify(actualScopeFlags) !== JSON.stringify(expectedScopeFlags)) {
            fail("scanScopeFlags", `expected ${JSON.stringify(expectedScopeFlags)}, got ${JSON.stringify(actualScopeFlags)}`);
            allOk = false;
        }
        else {
        ...
        if (!shop.accessTokenEncrypted ||
            ...
            !shop.accessTokenTag) {
            fail("accessTokenEncrypted", "encrypted / nonce / tag must all be non-empty");
            allOk = false;
        }
        else {
            pass(`accessTokenEncrypted = ${shop.accessTokenEncrypted.slice(0, 24)}... (${shop.accessTokenEncrypted.length} chars)`);
        }
        // Check 6: Decryption round-trip
        try {
            const decrypted = decryptToken(shop.accessTokenEncrypted, shop.accessTokenNonce, shop.accessTokenTag);
            if (decrypted !== expected.accessToken) {
                fail("decrypt round-trip", `expected "${expected.accessToken}", got "${decrypted}"`);
                allOk = false;
            }
            else {

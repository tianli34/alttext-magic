...
/** Shopify 订阅映射到本地的中间结构 */
interface MappedSubscription {
  planKey: PlanKey;
  /** 使用 Prisma BillingInterval 枚举（含 NONE） */
 ...
 */
function mapNameToPlanKey(name: string): PlanKey {
...
/**
  ...
  */
 function mapInterval(interval: 'EVERY_30_DAYS' | 'ANNUAL' | undefined): PrismaBillingInterval {
 ...
 */
function mapStatus(status: ActiveSubscription['status']): BillingSubscriptionStatus {
  switch (status) {
    case 'ACTIVE':
    case 'ACCEPTED':
      return 'ACTIVE';
 ...
 */
function computePeriodEnd(start: Date, interval: PrismaBillingInterval): Date {
  if (interval === 'ANNUAL') {
 ...
 */
function mapShopifySubscription(sub: ActiveSubscription): MappedSubscription {
...
export async function syncSubscriptionFromShopify(
  ...
  if (!subsResult.success) {
    throw new Error(
      `[subscription-service] 查询 Shopify 订阅失败: ${subsResult.errorMessage ?? 'Unknown'}`,
    );
  }

  // ---- 3. 找到第一个 ACTIVE 状态的 Shopify 订阅 ----
  const activeSub = subsResult.subscriptions.find(
    (s) => s.status === 'ACTIVE' || s.status === 'ACCEPTED' || s.status === 'FROZEN',
  );
  ...
  if (!activeSub) {
    ...
    return {
      ...
      planCode: (localActive?.planCode as PlanKey) ?? 'FREE',
  ...
  if (!isValidPlanKey(mapped.planKey)) {
    log.warn(
      { shopId: shop.id, subName: activeSub.name, mappedPlan: mapped.planKey },
      '无法映射 Shopify 订阅名称到本地计划，跳过',
    );
    const localActive = await db.billingSubscription.findFirst({
      where: { shopId: shop.id, status: 'ACTIVE' },
      select: { id: true, planCode: true, status: true },
  ...
  if (existing) {
    ...
    if (existing.status === mapped.status && existing.planCode === mapped.planKey) {
      log.info(
        { shopId: shop.id, subscriptionId: existing.id },
        '订阅已存在且状态一致，跳过（幂等）',
      );
      return {
        created: false,
    ...
    log.info(
      { shopId: shop.id, subscriptionId: existing.id, newStatus: mapped.status },
      '订阅状态更新完成',
    );

...
/** Free 月配额发放结果 */
export interface FreeMonthlyGrantResult {
  /** 扫描到的 Free 店铺总数 */
  totalFreeShops: number;
  /** 本次实际新发放的店铺数 */
  grantedCount: number;
  /** 已存在（跳过）的店铺数 */
  skippedCount: number;
...
/** 额度桶类型 */
const FREE_BUCKET_TYPE = 'FREE_MONTHLY_INCLUDED' as const;
 ...
 */
export function computeCycleKey(date?: Date): string {
 ...
 */
export async function grantFreeMonthlyToAllShops(
  targetMonth?: string,
  client?: PrismaClient,
): Promise<FreeMonthlyGrantResult> {
  // ---- 懒加载全局 Prisma 单例 ----
  ...
  if (targetMonth && !/^\d{4}-\d{2}$/.test(targetMonth)) {
    throw new Error(
      `[free-monthly-grant] targetMonth 格式无效: ${targetMonth}，期望 YYYY-MM`,
    );
  }
  ...
  for (const shop of shopsToGrant) {
    try {
      ...
      if (result.created) {
        grantedCount++;
        log.info(
          { shopId: shop.id, shopDomain: shop.shopDomain, cycleKey },
          'Free 月配额发放成功',
        );
      } else {
        skippedCount++;
      ...
      log.error(
        { shopId: shop.id, shopDomain: shop.shopDomain, cycleKey, err: error },
        'Free 月配额发放失败',
      );
      // 不中断循环，继续处理其他店铺
    }
  ...
  const result: FreeMonthlyGrantResult = {
    totalFreeShops: freeShops.length,
    grantedCount,

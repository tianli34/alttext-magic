...
/** 发起超额包购买入参 */
export interface InitiateOveragePackPurchaseParams {
  /** 店铺内部 ID */
  shopId: string;
  /** 店铺域名 */
  shopDomain: string;
  ...
  packCode: string;
  /** 基础回调 URL（服务会追加 purchaseId 参数） */
  returnUrl: string;
}

/** 发起购买结果 */
export interface InitiateOveragePackPurchaseResult {
  /** 确认支付跳转 URL */
 ...
 */
export function findOveragePackConfig(
  planKey: PlanKey,
  packCode: string,
): OveragePackConfig | null {
 ...
 */
export async function initiateOveragePackPurchase(
  params: InitiateOveragePackPurchaseParams,
  adapter: BillingAdapter,
  client: PrismaClient,
): Promise<InitiateOveragePackPurchaseResult> {
  const {
    shopId,
    shopDomain,
  ...
  if (!packConfig) {
    throw new Error(
      `[overage-pack] 计划 ${currentPlan} 不支持超额包 ${packCode}`,
    );
  }
  ...
  log.info(
    { shopId, packCode, purchaseId: purchase.id },
    '创建 PENDING 超额包购买记录',
  );
  ...
  if (!result.success || !result.confirmationUrl) {
    ...
    log.error(
      { shopId, packCode, purchaseId: purchase.id, error: result.errorMessage },
      'Shopify 购买创建失败',
    );

    throw new Error(
      `[overage-pack] Shopify 购买创建失败: ${result.errorMessage ?? 'unknown'}`,
    );
  }
...
export async function fulfillOveragePackPurchase(
  ...
  if (purchase.status !== 'PENDING') {
    throw new Error(
      `[overage-pack] 购买状态异常，期望 PENDING，实际: ${purchase.status}`,
    );
  }

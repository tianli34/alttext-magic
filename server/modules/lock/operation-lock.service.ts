...
interface ShopOperationLockRow {
  shop_id: string;
  lock_type: string;
...
export async function acquireLock(
  ...
  owner: ShopOperationLockOwner,
  options?: OperationLockTimingOptions,
  ...
  return prisma.$transaction(async (tx) => {
    ...
    if (existingLock === null) {
      ...
      if (insertedLock !== null) {
        logger.info(
          { shopId, operationType, batchId: owner.batchId, expiresAt },
          "Operation lock acquired by insert",
        );
...
export async function cleanupExpiredLocks(): Promise<CleanupExpiredLocksResult> {
  const now = new Date();
  const cleanedRows = await prisma.$queryRaw<Array<{ shop_id: string }>>(Prisma.sql`
...
interface HandleExistingLockInput {
  shopId: string;
  operationType: ShopOperationType;
  owner: ShopOperationLockOwner;
  now: Date;
...
async function handleExistingLock(
  ...
  if (
    ...
    logger.info(
      { shopId, operationType, batchId: owner.batchId, expiresAt },
      "Operation lock refreshed by same owner",
    );
  ...
  logger.info(
    { shopId, operationType, batchId: owner.batchId, previousStatus: currentLock.status },
    "Operation lock reclaimed",
  );
...
async function selectLockForUpdate(
  tx: TransactionClient,
  shopId: string,
): Promise<ShopOperationLockSnapshot | null> {
  const rows = await tx.$queryRaw<ShopOperationLockRow[]>(Prisma.sql`
    SELECT
      "shop_id",
      "lock_type",
      "batch_id",
...
async function updateLock(
  tx: TransactionClient,
  input: UpsertLockSqlInput,
): Promise<ShopOperationLockSnapshot> {
  const rows = await tx.$queryRaw<ShopOperationLockRow[]>(Prisma.sql`
    UPDATE "shop_operation_lock"
    SET
...
function mapRowToSnapshot(row: ShopOperationLockRow): ShopOperationLockSnapshot {
  return {
    shopId: row.shop_id,
...
function parseOperationType(value: string): ShopOperationType {
...
function parseLockStatus(value: string): ShopOperationLockStatus {

...
import {
  ...
  normalizeScanScopeFlags,
} from "../server/modules/shop/scope.service";
...
function run(): void {
  ...
  assert.deepEqual(
    normalizeScanScopeFlags(partial),
    partial,
    "合法 ScopeFlagState 应原样归一化返回",
  );

  // 全 false 也合法
  const allOff: ScanScopeFlags = {
    PRODUCT_MEDIA: false,
    FILES: false,
  ...
  assert.deepEqual(
    normalizeScanScopeFlags(allOff),
    allOff,
    "全 false 输入应合法",
  );
  ...
  // 3a. 缺少字段
  assert.throws(
    () => normalizeScanScopeFlags({ PRODUCT_MEDIA: true, FILES: true }),
    "缺少必需字段应抛错",
  );
  ...
  // 3d. 完全非法类型
  assert.throws(
    () => normalizeScanScopeFlags("not-an-object"),
    "字符串输入应抛错",
  );

  assert.throws(
    () => normalizeScanScopeFlags(null),
    "null 输入应抛错",
  );

  assert.throws(
    () => normalizeScanScopeFlags(123),
    "数字输入应抛错",
  );
  ...
  /* ================================================================ */
  const allOn: ScanScopeFlags = { ...DEFAULT_SCAN_SCOPE_FLAGS };
  ...
  const scanPartial: ScanScopeFlags = {
    PRODUCT_MEDIA: true,
    FILES: false,
    COLLECTION_IMAGE: true,
    ARTICLE_IMAGE: false,
  ...
  const effectivePartialScan = computeEffectiveReadScopeFlags(
    scanPartial,
    allOn,
  );

  assert.deepEqual(
    effectivePartialScan,
    scanPartial,
    "granted 全开、scan 部分开时，effective = scan",
  );

  // 5c. scanScopeFlags 全开，grantedReadScopes 部分开
  const grantedPartial: ScanScopeFlags = {
    PRODUCT_MEDIA: true,
    FILES: false,
    COLLECTION_IMAGE: true,
    ARTICLE_IMAGE: false,
  ...
  const effectivePartialGranted = computeEffectiveReadScopeFlags(
    allOn,
    grantedPartial,
  );

  assert.deepEqual(
    effectivePartialGranted,
    grantedPartial,
    "scan 全开、granted 部分开时，effective = granted",
  );

  // 5d. 两者各自开了不同的类型 —— 交集为空
  const scanAB: ScanScopeFlags = {
    PRODUCT_MEDIA: true,
    FILES: true,
    COLLECTION_IMAGE: false,
    ARTICLE_IMAGE: false,
  ...
  const grantedCD: ScanScopeFlags = {
    PRODUCT_MEDIA: false,
    FILES: false,
    COLLECTION_IMAGE: true,
    ARTICLE_IMAGE: true,
  ...
  // 5e. 两者部分重叠
  const scanOverlap: ScanScopeFlags = {
    PRODUCT_MEDIA: true,
    FILES: true,
    COLLECTION_IMAGE: false,
    ARTICLE_IMAGE: true,
  ...
  const grantedOverlap: ScanScopeFlags = {
    PRODUCT_MEDIA: true,
    FILES: false,
    COLLECTION_IMAGE: true,
    ARTICLE_IMAGE: true,
  ...
  const effectiveOverlap = computeEffectiveReadScopeFlags(
    scanOverlap,
    grantedOverlap,
  );

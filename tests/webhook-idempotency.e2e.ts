...
async function runTests() {
  ...
  try {
    ...
    assert(
      ...
      `isNew=${receipt1.isNew}, eventId=${receipt1.eventId}`,
    );
    assert(
      !!receipt1.eventId,
      "First call returns eventId",
      `eventId=${receipt1.eventId}`,
    );
    ...
    assert(
      dbRecord !== null,
      "Record exists in DB",
      `id=${dbRecord?.id}, status=${dbRecord?.status}`,
    );
    assert(
    ...
    assert(
      receipt2.isNew === false,
      "Replay isNew=false",
      `isNew=${receipt2.isNew}`,
    );
    assert(
      receipt2.eventId === firstEventId,
      "Replay returns same eventId",
      `eventId=${receipt2.eventId} (original=${firstEventId})`,
    );
    ...
    assert(
      countAfterReplay === 1,
      "Only 1 record in DB after replay",
    ...
    assert(
      receipt3.isNew === false,
      "receiveWebhook duplicate → isNew=false",
      `isNew=${receipt3.isNew}`,
    );
    ...
    const jobsForThisEvent = allJobsForId.filter(
      (j) => j?.id === firstEventId,
    );
    assert(
      jobsForThisEvent.length === 1,
      "Only 1 BullMQ job (no duplicate enqueue)",
      `jobCount=${jobsForThisEvent.length}`,
    );
    ...
    assert(
      !threwError,
      "Duplicate receiveWebhook does NOT throw",
      `threw=${threwError}`,
    );
    ...
    assert(
      receipt4.isNew === true,
      "Different webhook_id → isNew=true",
      `isNew=${receipt4.isNew}, eventId=${receipt4.eventId}`,
    );
    ...
    assert(
      job2 !== undefined,
      "Second job enqueued in BullMQ",
      `jobId=${receipt4.eventId}`,
    );
    ...
    const duplicateCount = [rapid1, rapid2, rapid3].filter(
      (r) => !r.isNew,
    ).length;

    assert(
      newCount === 1,
      "Exactly 1 of 3 concurrent calls is new",
      `newCount=${newCount}`,
    );
    assert(
      duplicateCount === 2,
      "Exactly 2 of 3 concurrent calls are duplicates",
      `duplicateCount=${duplicateCount}`,
    );

    // All 3 should return the same eventId
    const allSameId =
      rapid1.eventId === rapid2.eventId &&
      rapid2.eventId === rapid3.eventId;
    assert(
      allSameId,
      "All 3 concurrent calls return same eventId",
      `ids: ${rapid1.eventId}, ${rapid2.eventId}, ${rapid3.eventId}`,
    );
    ...
    assert(
      rapidDbCount === 1,
      "Only 1 DB record after 3 concurrent replays",
      `count=${rapidDbCount}`,
    );

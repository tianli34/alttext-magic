...
async function run(): Promise<void> {
  ...
  const {
    ...
    heartbeatLock,
    releaseLock,
  } = await import("../server/modules/lock/operation-lock.service.js");
  ...
  try {
    ...
    const conflictingAcquire = await acquireLock(
      shopId,
      "GENERATE",
      ownerGenerateB,
    );
    ...
    assert.equal(
      conflictingAcquire.lock.batchId,
      ownerScanA.batchId,
      "冲突时应返回当前持有者 batchId",
    );
    ...
    assert.equal(
      releaseMismatch.released,
      false,
      "非持有者 release 不应成功",
    );
    assert.equal(
      releaseMismatch.reason,
      "OWNER_MISMATCH",
      "非持有者 release 应返回 OWNER_MISMATCH",
    );
    ...
    const reacquiredAfterRelease = await acquireLock(
      shopId,
      "GENERATE",
      ownerGenerateB,
    );
    assert.equal(
      reacquiredAfterRelease.acquired,
      true,
      "release 后应可重新 acquire",
    );
    assert.equal(
      reacquiredAfterRelease.mode,
      "RECLAIMED",
      "复用已释放记录时应为 RECLAIMED",
    );
    assert.equal(
      reacquiredAfterRelease.lock.operationType,
      "GENERATE",
      "重新获取后锁类型应更新为 GENERATE",
    );
    ...
    const heartbeatedAcquire = await acquireLock(
      shopId,
      "WRITEBACK",
      ownerWritebackC,
      { ttlMs: 250 },
    ...
    assert.equal(
      lockAfterHeartbeatCleanup?.status,
      "RUNNING",
      "heartbeat 后 cleanup 不应提前回收当前锁",
    );
    assert.equal(
      lockAfterHeartbeatCleanup?.batchId,
      ownerWritebackC.batchId,
      "heartbeat 后 owner 不应变化",
    );

    const blockedByHeartbeat = await acquireLock(shopId, "SCAN", ownerScanD);
    assert.equal(
      blockedByHeartbeat.acquired,
      false,
      "heartbeat 续租后的锁仍应拦截其他操作",
    );
    assert.equal(
      blockedByHeartbeat.mode,
      "CONFLICT",
      "被续租锁拦截时应返回 CONFLICT",
    );
    ...
    assert.equal(
      cleanupResult.cleanedCount >= 1,
      true,
      "cleanup 应至少回收 1 条过期 RUNNING 锁",
    );
    ...
    assert.equal(
      lockAfterCleanup?.status,
      "EXPIRED",
      "超时 cleanup 后状态应为 EXPIRED",
    );
    assert.equal(
      lockAfterCleanup?.batchId,
      ownerScanE.batchId,
      "cleanup 不应篡改原过期锁 owner",
    );

    const reacquiredAfterCleanup = await acquireLock(
      shopId,
      "GENERATE",
      ownerGenerateF,
    );
    assert.equal(
      reacquiredAfterCleanup.acquired,
      true,
      "cleanup 回收后应可重新 acquire",
    );
    assert.equal(
      reacquiredAfterCleanup.mode,
      "RECLAIMED",
      "回收过期记录后重新获取应为 RECLAIMED",
    );
    assert.equal(
      reacquiredAfterCleanup.lock.batchId,
      ownerGenerateF.batchId,
      "重新获取后 owner 应更新",
    );

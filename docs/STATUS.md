...
- Task 9.5: 实现 `gdpr_delete` BullMQ Job（34 表拓扑顺序分批删除 + 幂等校验 + 结构化日志）；`gdpr-delete.queue.ts` 接口增加 `shopId`/`reason`；Worker 注册至 `worker/index.ts`；删除顺序文档 `docs/phase9-delete-order.md`

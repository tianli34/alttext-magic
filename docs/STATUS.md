...
- 本次：为 RUNNING 扫描增加 10 分钟无 Redis 进度更新的超时兜底；自动标记 FAILED、释放 SCAN 锁并清理 scan:progress Redis 键。
- 本次：SSE 扫描进度链路正式接入 UI。新增 `/app/scan-progress` 路由页，Dashboard 重新扫描/Onboarding 首次扫描后自动导航到进度页，实时展示 SSE 推送进度/阶段，完成时显示"扫描完成"与返回入口。Dashboard API 新增 `activeScanJobId` 字段，扫描中状态条增加"查看进度"按钮。移除所有 `window.location.reload()` 暴力刷新。
